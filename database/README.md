...
│   ├── test_sample_diaries.sql     # サンプル日記データ（旧版）
│   ├── seed_profiles.sql           # Seedデータ: ユーザープロフィール（Issue #267）
│   ├── seed_settings.sql           # Seedデータ: ユーザー設定（Issue #267）
│   ├── seed_diaries_full.sql       # Seedデータ: 日記エントリ充実版（Issue #267）
│   └── seed_diary_emotion_tags.sql # Seedデータ: 日記-感情タグ関連（Issue #267）
├── scripts/                        # 実行スクリプト（Issue #267）
│   ├── seed.sh                    # Seedデータ一括実行スクリプト
│   └── reset.sh                   # DB完全リセット + Seedデータ投入
...
### Seedデータ実行（ローカル開発環境）⚠️【Issue #267で追加】

ローカル開発環境でテストデータを一括投入する推奨方法：
...
- 日記エントリ: 75件 (各ユーザー15件、過去30日分)
- 感情タグマスタ: 20件
- 日記-感情タグ関連: 約75-225件

#### 🔄 **DB完全リセット + Seedデータ投入**
```bash
# DBを初期状態に戻してSeedデータを投入
./database/scripts/reset.sh
```

**注意:** `reset.sh`は既存データを全て削除します！

#### 📝 **テストユーザー情報**
- **alice** (theme: light, language: ja)
- **bob** (theme: dark, language: en)
- **charlie** (theme: light, language: ja)
- **diana** (theme: dark, language: ja)
- **eve** (theme: light, language: en)

パスワード: ローカル開発環境ではダミーハッシュを使用
