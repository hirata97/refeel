...
6. **PR作成** → 品質検証後、Claude Codeまたは `gh pr create` で作成
...
gh pr create                                           # 4. PR作成（Claude Codeまたはgh CLI）
...
## 📁 アーキテクチャ概要（Feature-based + Shared構造）

```
src/
├── features/           # 機能別モジュール（auth, dashboard, diary, mood等）
│   ├── auth/          # 認証・セキュリティ機能
...
│   ├── components/    # 共通コンポーネント（BaseButton, BaseCard等）
...
詳細: [docs/DEVELOPMENT/ARCHITECTURE.md](docs/DEVELOPMENT/ARCHITECTURE.md)
