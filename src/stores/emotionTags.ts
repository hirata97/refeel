...
export const useEmotionTagsStore = defineStore('emotionTags', () => {
  ...
  const linkDiaryEmotionTags = async (diaryId: string, emotionTagIds: string[]): Promise<void> => {
    try {
      ...
      if (emotionTagIds.length > 0) {
        ...
        const { error: insertError } = await supabase.from('diary_emotion_tags').insert(insertData)
  ...
  // 複数日記の感情タグを一括取得
  const getBatchDiaryEmotionTags = async (
    diaryIds: string[],
  ): Promise<Record<string, EmotionTag[]>> => {
