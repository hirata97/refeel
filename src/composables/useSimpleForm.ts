...
export const useSimpleRegisterForm = () => {
  ...
  // フォーム送信
  const handleSubmit = async (): Promise<{ username: string; email: string; password: string; confirmPassword: string } | null> => {
    if (!validateForm()) {
      return null
    }
...
export const useSimpleDiaryForm = () => {
  ...
  const handleSubmit = async (): Promise<{ title: string; content: string; date: string; mood: number } | null> => {
    if (!validateForm()) {
      return null
    }
  ...
  const isFormValid = computed(() => {
    return title.value && content.value && date.value && !titleError.value && !contentError.value && !dateError.value
  })
