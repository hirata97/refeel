...
|--------------|----------|-------------|------|-------|
| **PR Quality Gate** | `pr-quality-gate.yml` | PR作成・更新時 | 厳格な品質ゲート（PRマージ条件） | ⭐⭐⭐ 最重要 |
| Quality Check CI | `ci.yml` | push/PR (main, develop) | 基本的な品質チェック（監視・警告） | ⭐ 警告のみ |
...
```
PR作成 → PR Quality Gate（必須） → E2E Tests（推奨）
                                         ↓
                                    レビュー・承認
                                         ↓
                                    mainへマージ
...
### 1. PR Quality Gate（最重要）⭐⭐⭐

**ファイル**: `.github/workflows/pr-quality-gate.yml`
**実行タイミング**: PR作成・更新時（`opened`, `synchronize`, `reopened`）
**特徴**: 厳格モード（エラー時CI失敗、PRマージ不可）
...
### 2. Quality Check CI（基本品質チェック）⭐

**ファイル**: `.github/workflows/ci.yml`
**実行タイミング**: push/PR (main, develop)
**特徴**: `continue-on-error: true`（警告モード、CI失敗させない）

#### 実行ジョブ

| ジョブ名 | タイムアウト | 主要チェック | 失敗時の影響 |
|---------|------------|------------|------------|
| lint-and-format | 3分 | ESLint + Prettier | 警告のみ |
| type-check | 3分 | TypeScript型チェック | 警告のみ |
| unit-tests | 8分 | ユニットテスト（並列実行） | 警告のみ |
| build-check | 3分 | プロダクションビルド | 失敗時CI失敗 |
| security-audit | 3分 | npm audit | 警告のみ |
| ci-summary | - | 結果サマリー自動コメント | - |

**特徴**:
- `continue-on-error: true`により、エラーでもCI全体は成功扱い
- PR作成時のみCI Summaryコメント投稿
- 段階的品質改善のための監視目的

**キャッシュ戦略**:
- ESLint: `.eslintcache`
- TypeScript: `tsconfig.tsbuildinfo` + `node_modules/.cache`
- Vitest: `node_modules/.vitest` + `coverage/`
...
### 6. Auto Close Issues（Issue自動クローズ）
...
### 7. Issue Management Audit（Issue管理監査）
...
|----------|--------------|-------|----------|------------|
| **ESLint** | `pr-quality-gate.yml` | ⭐⭐⭐ 必須 | 全PR | CI失敗 |
|  | `ci.yml` | 警告のみ | push/PR (main, develop) | 警告表示 |
| **TypeScript型チェック** | `pr-quality-gate.yml` | ⭐⭐⭐ 必須 | 全PR | CI失敗 |
|  | `ci.yml` | 警告のみ | push/PR (main, develop) | 警告表示 |
|  | `type-generation.yml` | ⭐⭐ 必須 | スキーマ変更時 | CI失敗 |
|  | `deploy.yml` | ⭐⭐⭐ 必須 | デプロイ前 | デプロイ中止 |
| **Prettier** | `pr-quality-gate.yml` | ⭐⭐⭐ 必須 | 全PR | CI失敗 |
|  | `ci.yml` | 警告のみ | push/PR (main, develop) | 警告表示 |
| **ユニットテスト** | `pr-quality-gate.yml` | ⭐⭐⭐ 必須 | 全PR | CI失敗 |
|  | `ci.yml` | 警告のみ | push/PR (main, develop) | 警告表示 |
|  | `type-generation.yml` | ⭐⭐ 必須 | スキーマ変更時 | CI失敗 |
| **E2Eテスト** | `e2e-tests.yml` | ⭐⭐ 重要 | PR（src/変更時） | CI失敗 |
| **プロダクションビルド** | `pr-quality-gate.yml` | ⭐⭐⭐ 必須 | 全PR | CI失敗 |
|  | `ci.yml` | ⭐⭐ 重要 | push/PR (main, develop) | CI失敗 |
|  | `type-generation.yml` | ⭐⭐ 必須 | スキーマ変更時 | CI失敗 |
|  | `deploy.yml` | ⭐⭐⭐ 必須 | デプロイ前 | デプロイ中止 |
| **npm audit** | `pr-quality-gate.yml` | ⭐⭐⭐ 必須 | 全PR | CI失敗（High/Critical） |
|  | `ci.yml` | 警告のみ | push/PR (main, develop) | 警告表示 |
...
**PR Quality Gate（`pr-quality-gate.yml`）で強制**:
